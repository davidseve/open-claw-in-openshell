...
| `mlflow-openclaw plugin may not be loaded` | Re-run `scripts/launch-openclaw.sh` (reinstalls plugin, reapplies RHOAI MLflow wiring + patches) | #4, #5, #4b, #4c |
| `No traces in RHOAI MLflow`                | Re-run `scripts/wire-rhoai-mlflow-tracing.sh` then `scripts/launch-openclaw.sh` (RBAC/token/experiment may have expired or drifted) | #5, ADR-0018 |
| `Prompt trace linker not running`          | Re-run `scripts/launch-openclaw.sh` (restarts the linker with fresh RHOAI MLflow auth env vars from `.rendered/rhoai-mlflow/wiring.env` — do not hand-craft the `nohup node prompt-trace-linker.js` command manually, it now requires `MLFLOW_TRACKING_TOKEN`/`MLFLOW_WORKSPACE`/CA path) | #8         |
...
| `MLflow pod not ready`                     | `oc -n redhat-ods-applications rollout restart deployment mlflow` (RHOAI-managed, sole backend — see ADR-0018) | —          |
