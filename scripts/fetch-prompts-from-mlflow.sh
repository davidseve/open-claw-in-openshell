...
MLFLOW_URL="${MLFLOW_URL:-http://mlflow.observability.svc:5000}"
...
" 2>/dev/null)
  ...
  version_resp=$(curl -sf $CURL_OPTS \
...
while IFS='=' read -r key value; do
  [[ -z "$key" ]] && continue
  curl -sf $CURL_OPTS -X POST "${MLFLOW_URL}/api/2.0/mlflow/experiments/set-experiment-tag" \
