...
| Route LLM traffic through OpenShell's inference router (`inference.local`) instead of injecting credentials into the sandbox | Keeps the real API key exclusively in the gateway provider record; sandbox config uses `apiKey: "unused"` and never materializes secrets on disk | [ADR-0021](docs/adrs/ADR-0021-inference-router-migration.md), [constraints.md #3](docs/constraints.md#3-networking--llm-credential-injection-resolved-via-inference-router) |
...
| Scope narrow env-var workarounds to the exact flow that needs them | A global `OPENSHELL_GATEWAY_INSECURE=true` fixed OIDC token refresh on CRC but silently broke unrelated mTLS registration | [ROADMAP.md 13.5](ROADMAP.md#135-re-audit-openshell_gateway_insecure-scoping-when-aws-ocp-gets-real-certs-resolved) (resolved), CRC follow-ups in [Phase 18](ROADMAP.md#phase-18-future-crc-local-dev-improvements), [constraints.md #18](docs/constraints.md#18-openshell-cli-oidc-token-refresh-fails-on-crc--self-signed-router-ca-not-in-system-trust-store) |
