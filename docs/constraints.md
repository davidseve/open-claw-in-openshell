...
#26 leak (pre-ADR-0021 SecretRef era; now resolved by the inference router —
see constraint #26) and `CONFIG_PLACEHOLDER` correctly reported the real,
still-intact `${LITELLM_API_KEY}` SecretRef literal — both now driven by actual
grep results, not echoed input text. Post-ADR-0021, `CRED_WS` expects `CLEAN`
and the config check expects `apiKey: "unused"`.
...
## 26. OpenClaw's Per-Agent `models.json` Snapshot Wrote Resolved Plaintext apiKey to a Sandbox Path — RESOLVED (superseded by inference router)

**Status**: Resolved by ADR-0021 (inference router). No longer an active risk in this deployment.

**Historical finding (SecretRef era, pre-ADR-0021)**: When `openclaw.json` used
`apiKey: "${LITELLM_API_KEY}"` and the env var was injected into the sandbox,
OpenClaw also wrote the fully-resolved real `apiKey` to
`/sandbox/workspace/.openclaw/agents/main/agent/models.json`
(`src/agents/models-config.ts`). A plain `read` tool call returned the real key —
no jailbreak needed. `chmod 0600` did not help: gateway and agent tools run as the
same OS user (`sandbox`). Landlock and `tools.deny` could not path-scope deny it
(same limitation as constraint #24).

**Why it is no longer a problem**: With the inference router, `apiKey` is the
literal string `"unused"` and `LITELLM_API_KEY` is not injected into the sandbox.
`models.json` is regenerated with `"unused"`, not a real `sk-…` key. `verify.sh`
`CRED_WS` and the Layer 9 credential-leak Playwright test confirm no plaintext keys
on disk or in chat.

**Scripts affected**: `scripts/verify.sh` (Layer 4 `CRED_WS`, regression guard),
`scripts/launch-openclaw.sh` (clears stale `models.json` on restart — belt-and-suspenders
for SecretRef-era caches). Tracked in ROADMAP.md #13.4.
...
## 28. OCSF Audit Event Format — OpenShell Sandbox Proxy

**Context**: OpenShell's L7 sandbox proxy records every network policy
verdict as a structured [OCSF](https://schema.ocsf.io/) `HttpActivity`
event. These events are queryable via `openshell logs <sandbox> --source
sandbox` (or `--source gateway` for gateway-side events) and via
`openshell term` for live streaming.

**Event format** (observed from live sandbox logs):

```
ocsf: HTTP:<METHOD> [<SEVERITY>] <VERDICT> <METHOD> <URL> [policy:<policy_name> engine:<engine>] [reason:<reason_text>]
```

Fields:
- `METHOD`: HTTP method (`GET`, `POST`, `CONNECT`, etc.)
- `SEVERITY`: `INFO` (normal traffic), `MED` (denied by policy), `WARN`
  (system-level denial)
- `VERDICT`: `ALLOWED` or `DENIED`
- `URL`: the full URL the sandbox process attempted to reach
- `policy`: the sandbox policy block that matched (e.g. `observability`,
  `mlflow_direct`, `sandbox_service_routing`)
- `engine`: the policy engine that made the decision (`opa`, `gateway`,
  `nftables`)
- `reason` (DENIED only): human-readable explanation (e.g. `service
  endpoint not found`, `destination not in allow list`)

**Examples**:

```
ocsf: HTTP:GET [INFO] ALLOWED GET https://inference.local/v1/models [policy:inference engine:opa]
ocsf: HTTP:GET [MED] DENIED GET https://github.com/ [policy:default_deny engine:opa] [reason:destination not in allow list]
ocsf: HTTP:GET [MED] DENIED GET http://openclaw-ui.apps-crc.testing:8080/assets/foo.js [policy:sandbox_service_routing engine:gateway] [reason:service endpoint not found]
```

**Where these live**: OpenShell's own logging system, **not** in Tempo,
MLflow, or OpenShift's native audit log. They are a separate observability
layer specific to sandbox network security. See ROADMAP.md Phase 17 for
the planned integration with the OpenShift observability stack.

**verify.sh usage**: Layer 4 security tests assert both enforcement
(curl returns 403/timeout) AND audit trail (corresponding OCSF event
recorded). This catches silent audit failures where policy works but
logging is broken.

**Scripts affected**: `verify.sh` (Layer 4 OCSF assertions),
`policies/openclaw-sandbox.yaml` (defines the policy names that appear
in events).
